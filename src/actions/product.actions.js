...
export const createProduct = (productData) => (dispatch) => {
  ...
  fetch('http://localhost:3005/product/create', {
    method: 'POST',
...
export const getAllProducts = () => (dispatch) => {
  ...
  fetch('http://localhost:3005/product/products')
    .then((response) => response.json())
    .then((data) => dispatch({
...
export const getProductById = (id) => async (dispatch) => {
    ...
    try {
      const response = await fetch(`http://localhost:3005/product/products/${id}`);
...
export const updateProduct = (id, productData) => (dispatch) => {
  ...
  return fetch(`http://localhost:3005/product/update/${id}`, {
    method: 'PUT',
    body: formData,
  })
    .then(response => {
...
export const deleteProduct = (id) => (dispatch) => {
    ...
    fetch(`http://localhost:3005/product/delete/${id}`, {
        method: 'DELETE',
    })
    .then(response => {
        if (!response.ok) {
