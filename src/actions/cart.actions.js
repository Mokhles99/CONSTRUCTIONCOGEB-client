...
export const getCart = (id) => async (dispatch) => {
    ...
    try {
        const response = await fetch(`http://localhost:3005/cart/cart/${id}`);
        if (!response.ok) {
...
export const updateCart = (id, cartData) => async (dispatch) => {
    ...
    try {
        const response = await fetch(`http://localhost:3005/cart/updatecart/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
export const updateCartUserInfo = (cartId, userInfo) => async (dispatch) => {
    dispatch({ type: cartConstants.UPDATE_CART_REQUEST });
    try {
        const response = await fetch(`http://localhost:3005/cart/update/${cartId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
export const deleteCart = (id) => async (dispatch) => {
    ...
    try {
        const response = await fetch(`http://localhost:3005/cart/deletecart/${id}`, {
            method: 'DELETE'
        });
        if (response.ok) {
